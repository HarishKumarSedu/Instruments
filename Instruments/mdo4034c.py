...
class MDO4034C:
    """
    Tektronix MDO4034C Oscilloscope SCPI Control Class.
    """
    ...
    def set_trigger_edge_source(self, channel:int):
        self.inst.write(f"TRIG:EDGE:SOU CHAN{channel}")

    def get_trigger_edge_source(self):
        return self.inst.query("TRIG:EDGE:SOU?").strip()
    ...
    def get_trigger_edge_slope(self):
        return self.inst.query(f"TRIG:EDGE:SLOP?").strip()
    ...
    def enable_measurement(self, meas_num:int, meas_type:str, channel:int):
        ...
        self.inst.write(f"MEASU:MEAS{meas_num}:SOU CHAN{channel}")
